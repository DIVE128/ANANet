...
from utils.base_utils import save_h5, load_h5, load_component, load_cfg, get_stem, compute_precision_recall_np,setup_seed

from dataset.pose_dataset import PoseSequenceDataset, name2datalist
...
class PoseEvaluator:
    ...
    def filter_matches(self,dataset:PoseSequenceDataset,dataset_name,force=False):
        ...
        if not os.path.exists(filter_fn) or force:
            filter_dict={}
            ...
            for pair_id in tqdm(dataset.pair_ids):
                ...
                mask,_= self.filter(kps0, kps1, matches, img0, img1, filter_cur_name, K0, K1, desc0, desc1,
    ...
    def estimate_geom(self,dataset,force=False):
        det_fn=dataset.cache_fn(f'{self.det_name}')
        match_fn=dataset.cache_fn(f'{self.det_name}-{self.desc_name}-{self.matcher_name}')
        filter_fn = dataset.cache_fn(f'{self.det_name}-{self.desc_name}-{self.matcher_name}-{self.filter_name}')
        geom_fn=self.get_geom_fn(dataset)
        if not os.path.exists(geom_fn) or force==True:
    ...
    def eval_pose(self,dataset,force=False):
        geom_fn=self.get_geom_fn(dataset)
        error_fn=f'{geom_fn}-error'
        if not os.path.exists(error_fn) or force==True:
    ...
    def compute_metrics(self,dataset:PoseSequenceDataset,force=False):
        # load data
        geom_fn=self.get_geom_fn(dataset)
        error_fn=f'{geom_fn}-error'
        ...
        if self.filter is not None or (self.filter is not None and force):
    ...
    def eval_dataset(self,dataset,dataset_name):
        ...
        self.filter_matches(dataset,dataset_name,force)
        self.estimate_geom(dataset,force)
        self.eval_pose(dataset,True)
        error_array, pr_re_f1_array=self.compute_metrics(dataset,True)
        return error_array, pr_re_f1_array
    ...
    def __call__(self, dataset_name):
        ...
        print(msg)
        with open('data/pose_results.log','a') as file:
            # file.write(f'{self.eval_name:20} {dataset_name:20} {msg}\n')
            matcher_filter_name=f'{self.matcher_name}-{self.filter_name}' if self.filter is not None else self.matcher_name
            recoard_name=f'{self.det_name}-{self.desc_name}-{matcher_filter_name}-{self.estimator_name}'
            file.write(f'{dataset_name:20} {recoard_name:20} {msg}\n')
